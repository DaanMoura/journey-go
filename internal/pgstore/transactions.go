...
func (q *Queries) CreateTrip(
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: failed to begin transaction for CreateTrip. Details:  %w", err)
	}
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: failed to insert trip for CreateTrip. Details:  %w", err)
	}
	...
	if _, err := qtx.InviteParticipantsToTrip(ctx, participants); err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: failed to invite participants to trip for CreateTrip. Details:  %w", err)
	}

	if err := tx.Commit(ctx); err != nil {
		return uuid.UUID{}, fmt.Errorf("pgstore: failed to commit transaction for CreateTrip. Details:  %w", err)
	}
